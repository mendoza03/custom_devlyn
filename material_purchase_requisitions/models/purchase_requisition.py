...
#                raise UserError(_('You can not delete Purchase Requisition which is not in draft or cancelled or rejected state.'))
    ...
    state = fields.Selection([
        ...
        ('approve', 'Approved'),
        ('stock', 'Picking / Purchase Created'),
        ('receive', 'Received'),
        ('cancel', 'Cancelled'),
        ('reject', 'Rejected')],
        default='draft',
    ...
    def _prepare_po_line(self, line=False, purchase_order=False):
        ...
        budget_item = concept = task = False

        budget_item_line = purchase_order.budget_id.line_ids.filtered(
            lambda l: line.product_id.product_tmpl_id == l.product_id
        )
        if budget_item_line:
            budget_item = budget_item_line[0]
            parent_1 = budget_item.parent_id
            if parent_1 and parent_1.type == 'concept':
                concept = parent_1

                parent_2 = concept.parent_id
                if parent_2 and parent_2.type == 'task':
                    task = parent_2

        job_cost = job_cost_line = False
        job_costing = self.job_costing_id
        job_line_price = 0.0
        if job_costing and job_costing.state == 'approve':
            job_cost = job_costing

            job_cost_line_match = job_costing.job_cost_line_ids.filtered(
                lambda l: l.product_id == line.product_id
            )
            if job_cost_line_match:
                job_cost_line = job_cost_line_match[0]
                job_line_price = job_cost_line_match[0].cost_price
