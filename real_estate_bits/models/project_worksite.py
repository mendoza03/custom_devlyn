...
class Project(models.Model):
    ...
    is_enabled = fields.Boolean("Enabled", default=True)
    is_demo_worksite = fields.Boolean("Demo Worksite")
    is_sub_enabled = fields.Boolean("Sub Enabled", related="parent_id.is_enabled")
    # Project Fields
    ...
    converted_area = fields.Float("Converted Size", digits=(16, 2))
    ...
    total_units = fields.Integer(string="Total Properties", compute="_compute_total_units")
    ...
    count_condominium = fields.Integer(string="Quantity condominiums")

    condominium_count = fields.Integer(string="Condominium Count", compute='_compute_condominium_count')
    condominium_ids = fields.One2many(
        comodel_name="condominium.worksite",
        inverse_name="parent_id",
        string="Condominiums"
    )

    def _compute_condominium_count(self):
        for rec in self:
            rec.condominium_count = self.env['condominium.worksite'].search_count([('parent_id', '=', rec.id)])
    ...
    def _compute_total_area(self):
        ...
        area_field = 'property_area'  # CONFIRM THIS FIELD EXISTS

        for rec in self:
            if rec.parent_id:
                domain = [('project_worksite_id', '=', rec.id)]
            else:
                domain = [('worksite_id', '=', rec.id)]

            products = ProductTmpl.search(domain)
            ...
            total_area = sum(products.mapped(area_field)) if products else 0.0
            free_area = sum(free_products.mapped(area_field)) if free_products else 0.0
            sold_area = (
                    (sum(sold_products.mapped(area_field)) if sold_products else 0.0) +
                    (sum(reserved_products.mapped(area_field)) if reserved_products else 0.0)
            )

            rec.total_area = total_area
            rec.sold_area = sold_area
            rec.available_area = free_area
            rec.available_units = len(free_products)
            rec.sold_units = (len(sold_products) + len(reserved_products))

    @api.depends('sold_units', 'available_units')
    def _compute_total_units(self):
        for rec in self:
            rec.total_units = rec.sold_units + rec.available_units


    def action_view_condominiums(self):
        self.ensure_one()
        action = self.env.ref('real_estate_bits.property_condominium_form_action').read()[0]
    ...
    def action_create_condominium(self):
        for i in range(1, self.count_condominium + 1):
            values = self.read()[0]
            values.pop('id', None)
            values.pop('create_date', None)
            values.pop('write_date', None)
            values.pop('__last_update', None)
            values.pop('count_condominium', None)
            values.pop('message_follower_ids', None)
            values.pop('condominium_count', None)
            values.pop('commission_percentage', None)
            values.pop('commission_percentage_parent', None)
            values.pop('promotion_ids', None)
            values.pop('condominium_ids', None)
            values.pop('analytic_line_ids', None)
            values.pop('analytic_distribution', None)
            values.pop('analytic_precision', None)
            values.pop('x_studio_char_field_71f_1j1r9smdu', None)
            values.pop('x_studio_char_field_460_1j230qti9', None)
            values.pop('related_public_deeds', None)
            values.pop('recent_public_deed', None)
            values.pop('otherwise_clause', None)
            values.pop('compliance_clause', None)
            values.pop('company_legal_name', None)
            values.pop('company_legal_rep', None)
            values.pop('company_constitutive_act', None)
            values.pop('company_notary', None)
            values.pop('company_address', None)
            values.pop('company_email', None)
            values.pop('dgggf', None)
            values.pop('goal', None)
            values.pop('total_goal', None)
            values.pop('total_sell', None)
            values.pop('sell', None)
            values.pop('to_sell', None)
            values.pop('inventory', None)
            values.pop('total_available', None)
            values.pop('percent_sell_l', None)
            values.pop('percent_sell', None)
            values.pop('prom', None)
            values.pop('total_income', None)
            values.pop('total_condominios', None)
            values.pop('total_property_area', None)
            values['parent_id'] = self.id
            values['main_project_id'] = self.id
            for key, value in values.items():
                if isinstance(value, tuple) and len(value) == 2:
                    values[key] = value[0]

            values['name'] = self.name + '-' + str(i)
            values['default_code'] =  self.default_code + '-' + str(i)

            self.env['condominium.worksite'].with_context(mail_create_nosubscribe=True).create(values)
            self._compute_condominium_count()
