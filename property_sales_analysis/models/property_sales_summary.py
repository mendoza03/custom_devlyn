...
class PropertySalesSummary(models.Model):
    _name = "property.sales.summary"
    _description = "Property Sales Summary - Annual View"
    _order = "cluster_id, worksite_id"
    ...
    # Weekly fields - Units sold (53 weeks)
    ...
    # Weekly fields - Manager details (53 weeks)
    ...
    @api.depends("worksite_id")
    def _compute_name(self):
        for record in self:
            if record.worksite_id and record.calendar_id:
                record.name = f"{record.worksite_id.name} - {record.calendar_id.name}"
            elif record.worksite_id:
                record.name = record.worksite_id.name
            else:
                record.name = "N/A"
    ...
    def _compute_totals(self):
        for record in self:
            ...
            # Obtener todos los condominiums del worksite
            condominiums = self.env["condominium.worksite"].search([
                ("parent_id", "=", record.worksite_id.id)
            ])
            ...
            # Buscar TODOS los lotes de esos condominiums
            domain = [
                ("is_property", "=", True),
    ...
    @api.depends("worksite_id", "calendar_id")
    def _compute_weekly(self):
        for record in self:
            ...
            if not record.worksite_id or not record.calendar_id:
                continue
            ...
            # Obtener TODOS los meses del calendario primero
            ...
            # Obtener TODAS las semanas ordenadas por mes y luego por número de semana dentro del mes
            all_weeks = []
            ...
            # Calcular ventas por cada semana usando NUMERACIÓN SECUENCIAL
            for sequential_num, week in enumerate(all_weeks, 1):
                # sequential_num es 1, 2, 3, 4... 53 (posición en el año)
                
                # Solo procesar semanas 1-53
                ...
                # Buscar propiedades vendidas en esta semana
                ...
                # Si hay ventas, agrupar por gerente
                if count > 0:
                    manager_data = {}
    ...
    def action_debug_weekly(self):
        """DEBUG: Ver qué está pasando con el cálculo semanal"""
        self.ensure_one()
        
        # Obtener condominiums
        condominiums = self.env["condominium.worksite"].search([
            ("parent_id", "=", self.worksite_id.id)
        ])
        
        # Obtener propiedades apartadas
        properties = self.env["product.template"].search([
            ("is_property", "=", True),
            ("condominium_worksite_id", "in", condominiums.ids),
            ("state", "=", "reserved"),
        ...
        msg = f"🔍 DEBUG INFO\n\n"
        msg += f"📊 DATOS BÁSICOS:\n"
        msg += f"Condominiums: {len(condominiums)}\n"
        msg += f"Propiedades vendidas: {len(properties)}\n\n"
        
        msg += f"📅 PROPIEDADES CON FECHA:\n"
        props_with_date = properties.filtered(lambda p: p.date_state_change)
        msg += f"Con fecha: {len(props_with_date)}\n"
        msg += f"Sin fecha: {len(properties) - len(props_with_date)}\n\n"
        
        if props_with_date:
            msg += "Últimas 5 con fecha:\n"
            for prop in props_with_date[:5]:
                msg += f"  • {prop.name}: {prop.date_state_change}\n"
        
        # Obtener semanas
        calendar_months = self.env["sale.goal.calendar.month"].search([
            ("calendar_id", "=", self.calendar_id.id)
        ])
        all_weeks = self.env["sale.goal.calendar.week"].search([
            ("month_id", "in", calendar_months.ids)
        ], order="week_number")
        
        msg += f"\n📆 SEMANAS EN CALENDARIO: {len(all_weeks)}\n\n"
        
        # Mostrar semanas 5-8 (febrero)
        msg += "Semanas de Febrero:\n"
        for week in all_weeks[4:8] if len(all_weeks) >= 8 else all_weeks:
            msg += f"  S{week.week_number}: {week.start_date} → {week.end_date}\n"
        
        # Buscar en qué semana cae 09/02/2026
        from datetime import date
        target_date = date(2026, 2, 9)
        matching_week = all_weeks.filtered(lambda w: w.start_date <= target_date <= w.end_date)
        
        msg += f"\n🎯 VERIFICACIÓN 09/Feb/2026:\n"
        if matching_week:
            msg += f"✅ Está en S{matching_week[0].week_number}\n"
            msg += f"   Rango: {matching_week[0].start_date} → {matching_week[0].end_date}\n"
            
            # Contar ventas en esa semana
            start_dt = datetime.combine(matching_week[0].start_date, datetime.min.time())
            end_dt = datetime.combine(matching_week[0].end_date, datetime.max.time())
            ...
            msg += f"   Ventas en S{matching_week[0].week_number}: {count}\n"
        else:
            msg += f"❌ 09/Feb/2026 NO está en ninguna semana\n"
        
        raise models.UserError(msg)