...
class SaleEstimateJob(models.Model):
    ...
    def _prepare_quotation_line(self, quotation):
        quo_line_obj = self.env['sale.order.line']
        sequence = 10  # Initial sequence, generally Odoo ma 10 thi start thay che

        for rec in self:
            all_lines = []
            
            for line in rec.estimate_ids + rec.labour_estimate_line_ids + rec.overhead_estimate_line_ids:
                ...
                quo_line = quo_line_obj.create(vals)
                all_lines.append(quo_line)

            # Update sequence properly after creation
            for line in all_lines:
                line.sequence = sequence
                sequence += 10  # Increase for next line
