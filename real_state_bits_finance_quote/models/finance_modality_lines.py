...
class ModalityFinanceLines(models.Model):
    ...
    def onchange_discount(self):
        if self.promotion_id:
            discount_percent = self.amount_total * self.promotion_id.porcent
            discount_bono = self.promotion_id.porcent_bono

            amount_after_discount = self.amount_total - discount_percent

            hitch_from_percent = amount_after_discount * self.order_id.hitch_porcent

            discount_hitch = hitch_from_percent * self.promotion_id.porcent_financial
            ...
            values = self.order_id._get_paymemt_lines(self.interest_id.payment_term_ids, self.amount_finance)
            print("FINANCE CALC payment_values=", values)

            difer_hitch_id = self._origin.order_id.hitch_ids.filtered(lambda x: x.finance_id.id == self._origin.id)
            if difer_hitch_id:
                difer_hitch_id.write({'amount': self.hitch / difer_hitch_id.months})
