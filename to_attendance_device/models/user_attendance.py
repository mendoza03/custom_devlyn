...
class UserAttendance(models.Model):
    ...
    _user_device_timestamp_unique = models.Constraint(
    ...
    def _sync_attendance(self):
        ...
        for attendance_activity in self.activity_id:
            for employee in self.filtered(lambda r: r.activity_id == attendance_activity).employee_id:
                ...
                first_hr_attendance = self.env['hr.attendance'].search([
                    ('employee_id', '=', employee.id),
                    ('checkin_device_id', '!=', False),
                    ('check_in', '<=', unsync_user_attendances[0].timestamp),
                ...
                hr_attendances = self.env['hr.attendance'].search([
                    ('employee_id', '=', employee.id),
                    ('checkin_device_id', '!=', False),
                    ('check_in', '>=', first_hr_attendance.check_in),
                    ('activity_id', 'in', [attendance_activity.id, False]),
                    ], order='check_in'
                )
        ...
        if bool(error_msg):
            for device, msg_list in error_msg.items():
                device.message_post(body=Markup("<ol>%s</ol>" % "".join(["<li>%s</li>" % msg for msg in msg_list])))
    ...
    def _cron_synch_hr_attendance(self):
        employee = self.env['user.attendance'].search(self._prepare_unsynch_data_domain(), limit=1).employee_id
        if not employee:
            return True
        domain = Domain.AND([self._prepare_unsynch_data_domain(), [('employee_id', '=', employee.id)]])
        unsync_data = self.env['user.attendance'].search(domain)
        unsync_data._sync_attendance()
        if self.env['user.attendance'].search(self._prepare_unsynch_data_domain(), limit=1):
            self.env.ref('to_attendance_device.ir_cron_scheduler_sync_attendance')._trigger()
