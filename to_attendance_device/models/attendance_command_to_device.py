...
class CommandToDevice(models.Model):
    ...
    @api.model
    def _cron_delete_old_commands(self, days=180):
        batch_size = 1000
        cmd = self.env['attendance.command.device'].search(
            [('return_time', '<', fields.Datetime.now() - timedelta(days=days))],
            limit=batch_size + 1
        )
        to_process = cmd[:batch_size]
        if to_process:
            to_process.unlink()
        if len(cmd) > batch_size:
            self.env.ref('to_attendance_device.ir_cron_scheduler_delete_old_commands')._trigger()
        return True
