...
class AttendanceState(models.Model):
    ...
    activity_id = fields.Many2one('attendance.activity', string='Activity', required=True,
                                  help="Attendance activity, e.g. Normal Working, Overtime, etc", tracking=True, index=True)
    ...
    _code_unique_constraint = models.Constraint(
        'unique(code)',
        "The Code must be unique!",
    )
    _name_activity_unique_constraint = models.Constraint(
        'unique(name, activity_id)',
        "The state name must be unique within the same activity!",
    )
    _type_activity_unique_constraint = models.Constraint(
    ...
    def name_search(self, name='', domain=None, operator='ilike', limit=100):
        ...
        domain = domain or []
        if name:
            args = ['|', ('activity_id.name', '=ilike', name + '%'), ('name', operator, name)]
