...
class HrEmployee(models.Model):
    ...
    unamapped_attendance_device_ids = fields.Many2many('attendance.device', 'device_employee_rel', 'employee_id', 'device_id',
                                                       string='Unmapped Machines', groups="hr.group_hr_user",
    ...
    finger_templates_ids = fields.One2many('finger.template', 'employee_id', string='Finger Template', groups='hr.group_hr_user', readonly=True)
    total_finger_template_records = fields.Integer(string='Finger Templates', compute='_compute_total_finger_template_records', groups='hr.group_hr_user,to_attendance_device.group_attendance_devices_manager')
    bio_templates_ids = fields.One2many('bio.template', 'employee_id', string='Bio Template', groups='hr.group_hr_user', readonly=True)
    total_bio_template_records = fields.Integer(string='Biometrics Templates', compute='_compute_total_bio_template_records', groups='hr.group_hr_user,to_attendance_device.group_attendance_devices_manager')
    device_user_ids = fields.One2many('attendance.device.user', 'employee_id', string='Mapped Device Users', groups='hr.group_hr_user')
    ...
    def upload_to_attendance_device(self, device):
        ...
        for r in self:
            try:
                ...
                error_msg += _("Fail to upload the employee: %(name)s to the machine: %(machine)s. %(err)s\n",
                               name=r.name, machine=device.name, err=e)
        return error_msg
