...
class ZkDeviceMock():
    ...
    attendances = [
        Attendance(user_id='1', timestamp=fields.Datetime.now(), status=0, punch=0, uid=1),
        Attendance(user_id='1', timestamp=fields.Datetime.now() + timedelta(hours=8), status=0, punch=1, uid=1),
        Attendance(user_id='1', timestamp=fields.Datetime.now() + timedelta(days=1), status=0, punch=0, uid=1),
        Attendance(user_id='1', timestamp=fields.Datetime.now() + timedelta(days=1.5), status=0, punch=1, uid=1),
    ]
...
class TestAttendanceDeviceMock(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.attendance_device_location = cls.env['attendance.device.location'].create({
            ...
            'hr_work_location_id': cls.work_office_1.id,
            'tz': 'Asia/Ho_Chi_Minh',
            })

        cls.user_01 = cls.env['res.users'].create({
            'name': 'user_01',
        ...
        cls.user_02 = cls.env['res.users'].create({
            'name': 'user_02',
        ...
        cls.emp01 = cls.env['hr.employee'].create({
            'user_id': cls.user_01.id,
            'barcode': '123456',
        })

        cls.emp02 = cls.env['hr.employee'].create({
            'user_id': cls.user_02.id,
            'barcode': '234567',
        })
    ...
    def test_09_action_attendance_download(self):
        ...
        self.assertEqual(self.attendance_device.total_att_records, 4, "test_action_attendance_download failed")

    # 20. Upload nhân viên lên thiết bị chấm công:
    def test_10_action_employee_upload_to_device(self):
        employee_to_upload = self.env['hr.employee'].create({
            'name': 'Richard',
            'sex': 'male',
    ...
    def test_11_sync_attendance(self):
        ...
        self.assertEqual(self.env['hr.attendance'].search_count([]), 2, "test_sync_attendance failed")
    ...
    def test_20_compute_total_attendance_records(self):
        ...
        self.attendance_device._compute_total_attendance_records()
        self.assertEqual(self.attendance_device.total_att_records, 5, "test_compute_total_attendance_records failed")
    ...
    def test_24_action_attendance_download_devices(self):
        ...
        self.assertEqual(self.attendance_device.total_att_records, 4)
        self.assertEqual(self.attendance_device_2.total_att_records, 4)
        self.assertEqual(self.attendance_device_3.total_att_records, 4)
