...
class Icloud(http.Controller):
    ...
    def returned_data_from_device(self, SN, **kw):
        ...
        pushver = kw.get('pushver', None)
        try:
            data = request.httprequest.data.decode('utf-8')
        except Exception:
            data = request.httprequest.data.decode('gb18030')
        res = request.env['attendance.device'].sudo().process_data_from_device(method, SN, data, table, pushver)
        return Response(res)
    ...
    def ping(self, SN, **kw):
        res = request.env['attendance.device'].sudo().process_ping(SN)
        if res:
            return Response(res)
